...
import { getReadLaterList, removeFromReadLater, type ReadLaterItem } from '../api/readlater'
...
// 将ReadLaterItem转换为NewsCard所需的格式
const transformReadLaterItemToNewsItem = (item: ReadLaterItem) => {
  return {
    id: item.id,
    title: item.title,
...
const loadReadLaterNews = async () => {
  ...
  try {
    ...
    const response = await getReadLaterList({
      pageNum: 1,
      pageSize: 1000, // 假设用户不会有超过1000个稍后再读项目
    })

    if (response.code === 200 && response.data) {
      // 转换数据格式
      news.value = response.data.map(transformReadLaterItemToNewsItem)
      // 应用筛选和搜索
      applyFiltersAndSearch()
    } else {
      error.value = response.message || '获取稍后再读列表失败'
      news.value = []
    }
  } catch (err: any) {
    error.value = err.message || '获取稍后再读列表失败，请稍后再试'
    news.value = []
  } finally {
    loading.value = false
  }
...
const handleRemoveItem = async (itemId: number) => {
  try {
    loading.value = true
    const response = await removeFromReadLater(itemId.toString())

    if (response.code === 200) {
      // 删除成功后重新获取最新的稍后再读列表
      selectedItems.value.delete(itemId)
      await loadReadLaterNews()
    } else {
...
const batchDeleteSelected = async () => {
  ...
  if (!confirm(`确定要删除选中的 ${selectedItems.value.size} 篇文章吗？`)) {
    return
  }
  ...
  try {
    ...
    const deletePromises = Array.from(selectedItems.value).map((id) =>
      removeFromReadLater(id.toString()),
    )
    ...
    error.value = err.message || '批量删除失败，请稍后再试'
  } finally {
    loading.value = false
  }
}
...
const clearAllItems = async () => {
  ...
  if (!confirm(`确定要清空所有 ${news.value.length} 篇稍后再读的文章吗？此操作不可撤销。`)) {
    return
  }

  try {
    loading.value = true
    const deletePromises = news.value.map((item) => removeFromReadLater(item.id.toString()))
    ...
    <main class="main-content">
      ...
      <div class="content">
        ...
        <div v-else-if="isEmpty" class="empty-state">
          ...
          <h3>暂无稍后再读的新闻</h3>
          <p>当您在浏览新闻时点击"稍后再读"按钮，文章就会出现在这里</p>
        </div>
